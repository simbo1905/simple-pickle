...
No Framework Pickler came about because:
...
- Nested `sealed interfaces` that only contain `records` then we can use exhaustively matched switch statements to deconstruct the records.

So we no longer need complex kitchen sink frameworks to have an elegant way to do create a binary protocol for data transfer objects.
...
The fact that you are spoilt for choices is not a good thing. It shows that the mapping between arbitrary Java types and standard protocols is hard. Yet we can map Java's built-in `record` types which are intended to be "data transfer objects" then all the complexity drops away.

So why something that can potentially free thousands of companies from thousands of hours of build time code end up being in a single Java file? Well, I just wanted something quick and dirty for a very simple record protocol when I didn't want to pull in another framework and kinda fell down a rabbit hole. 
